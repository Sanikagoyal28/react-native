...
const SigninSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(SignInThunk.fulfilled, async (state, action) => {
      state.toast = true;
      state.loading = false;
      if (action.payload.data.sucess) {
        ...
        const newToken = JSON.stringify(action.payload.data.token);
    ...
    builder.addCase(OtpVerifyThunk.fulfilled, async (state, action) => {
      state.toast = true;
      state.loading = false;
      if (action.payload.data.success) {
        state.response = action.payload.data.msg;
    ...
    builder.addCase(SignupVerifyThunk.fulfilled, async (state, action) => {
      state.loading = false;
      if (action.payload.data.success) {
        state.response = action.payload.data.msg;
        state.error = "";
