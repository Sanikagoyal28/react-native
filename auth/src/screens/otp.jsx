  ...
  function handleSubmit() {
    ...
    if (otp && isValid) {
      ...
      if (identifier == "signUp") {
        dispatch(SignupVerifyThunk(data)).then((res) => {
          setIsApi(false);
        ...
        dispatch(OtpVerifyThunk(data)).then((res) => {
          setIsApi(false);
          if (res.payload.data.success) {
            Toast.show({
              type: ALERT_TYPE.SUCCESS,
      ...
      <AlertNotificationRoot>
        <View style={{ backgroundColor: GlobalColors.bg, flex: 1 }}>
          ...
          <View className="py-8 px-8 flex flex-col justify-center items-center">
            <Text className="text-4xl mb-2 text-white font-extrabold text-center leading-[58px]">
              {title}
            </Text>
            <Text className="text-base text-gray-200 my-4 w-full leading-5">
            ...
            <View
              className="rounded-lg px-2 py-3 w-full mt-4 mb-2"
